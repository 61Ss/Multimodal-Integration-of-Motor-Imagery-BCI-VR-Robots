...
class FullExactMutualInformationAnalyzer:
    ...
    def calculate_average_mi_and_rank_channels(self, band_mi_matrices):
        ...
        # 排序获取前4个通道
        sorted_channels_indices = np.argsort(channel_connectivity)[::-1]
        sorted_channels_scores = channel_connectivity[sorted_channels_indices]
        sorted_channels_names = [self.ch_names[i] for i in sorted_channels_indices]
        
        return avg_mi_matrix, sorted_channels_indices, sorted_channels_scores, sorted_channels_names
    ...
    def run_full_exact_analysis(self, resume=True):
        ...
        for trial_idx in range(start_trial, num_trials):
            ...
            avg_mi_matrix, sorted_indices, sorted_scores, sorted_names = self.calculate_average_mi_and_rank_channels(band_mi_matrices)
        ...
        for result in results:
            ...
            # 添加所有通道的信息
            for i, (channel_name, score) in enumerate(zip(result['sorted_channels'], result['sorted_scores'])):
                row[f'channel_{i+1}'] = channel_name
                row[f'channel_{i+1}_score'] = score
