import re
import pandas as pd
import matplotlib.pyplot as plt
from matplotlib.lines import Line2D

# 1. 读取 CSV 并重命名列
df = pd.read_csv('electrode_coords.csv')
df = df.rename(columns={
    'channel_label': 'name',
    'x_position':    'x',
    'y_position':    'y'
})

# 2. 提取 2D 坐标和通道名
coords2d = df[['x', 'y']].to_numpy()   # shape = (118, 2)
names2d  = df['name'].tolist()         # list of 118 channel names

# 3. 在运动想象 BCI 场景下，按左右感觉运动区分
#    - 基于 10–20 系统：前缀 C（中央区）+ 通道编号的奇偶性
#      odd → 左感觉运动区 (Left-SM)
#      even → 右感觉运动区 (Right-SM)
#    其他电极归为 Other
ch_regions = []
for ch in names2d:
    m = re.search(r'(\d+)$', ch)  # 提取通道名末尾的数字
    if ch.startswith('C') and m:
        num = int(m.group(1))
        if num % 2 == 1:
            ch_regions.append('Left-SM')
        else:
            ch_regions.append('Right-SM')
    else:
        ch_regions.append('Other')

unique_regs = sorted(set(ch_regions))
cmap       = plt.get_cmap('tab10')
color_map  = {reg: cmap(i) for i, reg in enumerate(unique_regs)}
colors     = [color_map[r] for r in ch_regions]

# 4. 绘制散点并标注
plt.figure(figsize=(7,7))
plt.scatter(coords2d[:,0], coords2d[:,1],
            c=colors, s=80, edgecolors='k')
for (x, y), ch in zip(coords2d, names2d):
    plt.text(x, y, ch, ha='center', va='center', fontsize=7)

# 5. 添加图例
legend_handles = [
    Line2D([0], [0], marker='o', color='w',
           markerfacecolor=color_map[reg], markersize=8,
           label=reg)
    for reg in unique_regs
]
plt.legend(handles=legend_handles, title='Region',
           loc='upper right', frameon=False)

plt.title('118 EEG Electrodes: Left vs Right Sensorimotor Areas', pad=15)
plt.axis('off')
plt.tight_layout()
plt.show()
